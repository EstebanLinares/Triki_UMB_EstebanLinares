...
MiniMax.marcarTablero = function (turno, posicion) {
...
MiniMax.min = function() {
    ...
    for (var i = 0; i < n; i++)
        if (this.tableroMarcable(i)) {
            this.marcarTablero(this.JUGADOR.HUMANO, i);
            aux = this.max();
            if (aux < mejor)
                mejor = aux;
            this.marcarTablero(0, i)
        }
    return mejor
}
...
MiniMax.max = function() {
    ...
    for (var i = 0; i < n; i++)
        if (this.tableroMarcable(i)) {
            this.marcarTablero(this.JUGADOR.CPU, i);
            aux = this.min();
            if (aux > mejor)
                mejor = aux;
            this.marcarTablero(0, i);
...
MiniMax.getPos = function(ar) {
    ...
    this.marcarTablero(this.JUGADOR.CPU, posicion);
